...
Further queue-index and cache-copy experiments preserved correctness but did not improve the replay benchmark, so neither was integrated. An opt-in native upper-arithmetic prototype now passes 122/122 VU-related tests and reproduces all 32 captured workloads, with every captured upper instruction taking the native path. Nine alternating comparisons reduce median replay execution time from 4476.139 ms to 4164.376 ms, about 7%. This is an offline prototype, not a gameplay FPS improvement or a complete compiled-VU engine. It remains disabled by default and is not loaded by the game; the last validated gameplay executable is unchanged.
