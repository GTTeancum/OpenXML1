...
The September 6 candidate integrates verified work through PS2Recomp `280147c`, including overlapping-register-write correctness, flag packing, native normalized-operand reuse, and removal of redundant initialization copies. Direct clearing of pending VU entries improves two local replay workloads by **3.4% and 6.4%**, with exact results and seven wins in seven alternating comparisons each. The standalone change is submitted as [PS2Recomp PR #252](https://github.com/ran-j/PS2Recomp/pull/252), with 426/426 upstream-based tests passing before and after it. These are VU-only measurements, not gameplay FPS gains.

The updated candidate reaches textured New York and Wolverine through the real New Game handler. It measured **4.38 FPS** over 128 frames and exited normally; the preceding candidate run measured 4.14 FPS. This is still not practical playability, and these shared-host observations do not establish a sustained improvement. A runtime-owned frame confirms the known black props, missing foliage, and HUD/effect defects remain. The candidate remains opt-in and the saved primary/staged builds are unchanged.

The latest gameplay phase profile, from an earlier candidate, attributes about **64.0%** of measured wall time to VU processing and **24.3%** to graphics submission/drawing; waiting is negligible. Gameplay-only coverage shows **33.8% of vector instruction pairs still use the interpreter**, while 45.3% run in compiled blocks and 20.9% in smaller native kernels. These coverage counts are not time shares. Practical frame rate remains the main unfinished requirement. The latest normal build passes **139/139 VU-related tests** and both private captures at normal and 1/8/16/64-cycle slicing. This is not a full-suite pass or proof of gameplay fidelity.

The newest optimization removes a redundant 64 KiB temporary clear/copy from every graphics-packet launch. Two recorded VU workloads improved by **15.5% and 17.7%**, winning all seven alternating comparisons each while preserving exact state, packet bytes, and cycles. The integrated candidate's runtime-owned framebuffer still shows the known scene defects; no rendering correction or sustained whole-game speedup is claimed. Earlier kernel-selection, operand-reuse, and deferred-writeback experiments with mixed or negative timings remain excluded. Detailed results are in [VU-REPLAY.md](xmen-legends/VU-REPLAY.md).
