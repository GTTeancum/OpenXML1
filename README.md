...
Further queue-index and cache-copy experiments preserved correctness but did not improve the replay benchmark, so neither was integrated. The next major performance target is compiled VU execution, with offline verification before gameplay integration. The last validated gameplay executable remains unchanged.
