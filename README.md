...
The September 6 candidate integrates verified work through PS2Recomp `1395fb9`. The latest change processes four native arithmetic result lanes together, preserving their exact classification and rounding behavior. Two local replay workloads improve by **5.1% and 5.5%**, with exact results and seven wins in seven alternating comparisons each. The earlier independent pending-entry optimization is submitted as [PS2Recomp PR #252](https://github.com/ran-j/PS2Recomp/pull/252), with 426/426 upstream-based tests passing before and after it. These are VU-only measurements, not gameplay FPS gains.

The updated candidate reaches textured New York and Wolverine through the real New Game handler. It measured **4.74 FPS** over 128 frames and exited normally; the preceding candidate run measured 4.63 FPS. This is still not practical playability, and these shared-host observations do not establish a sustained improvement. A runtime-owned frame confirms the known black props, missing foliage, and HUD/effect defects remain. The candidate remains opt-in and the saved primary/staged builds are unchanged.

The latest gameplay phase profile, from an earlier candidate, attributes about **59.6%** of measured wall time to VU processing and **28.2%** to graphics submission/drawing; scheduler reset is under 1%. Earlier gameplay-only coverage shows **33.8% of vector instruction pairs still use the interpreter**, while 45.3% run in compiled blocks and 20.9% in smaller native kernels. These coverage counts are not time shares. Practical frame rate remains the main unfinished requirement. The latest normal build passes **143/143 VU-related tests** and both private captures at normal and 1/8/16/64-cycle slicing. Arithmetic comparisons also check restoration of the caller's rounding mode after console-mode execution. This is not a full-suite pass or proof of gameplay fidelity.
