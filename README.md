...
This is not yet a playable-speed build: the latest ordinary run measured about 3.25 FPS over 128 gameplay frames, without performance profilers enabled. That is better than the preceding 3.12 FPS sample, but it is not a controlled sustained-speed guarantee. Broader timing identifies vector processing as the main remaining cost. A small operand-preparation optimization is submitted as [PS2Recomp PR #247](https://github.com/ran-j/PS2Recomp/pull/247), with 427/427 upstream-based tests. Removing duplicate cycle work, improving packet copying, and replacing full integer-register scans with existing masks is submitted as [PS2Recomp PR #248](https://github.com/ran-j/PS2Recomp/pull/248), with 427/427 upstream-based tests. The current bring-up passes 101/101 focused vector tests and 119/119 VU-related tests; this is not a full-suite pass.
...
Whole-block VU execution is now implemented and opt-in, with 54/54 focused tests passing and exact results over the 32 captured workloads, including short-cycle resumption. Live integration is pending: the complete generated-game compilation succeeded, but linking exceeded the unchanged 2 GiB memory limit. A separate runner-IPO option is configured to reduce linker memory while keeping runtime/native-kernel optimization enabled; that configuration still needs its full game rebuild and live comparison. Offline improvements are not a new gameplay FPS result. See [the current resume point](xmen-legends/TODO.md) and [validation notes](xmen-legends/VU-REPLAY.md).
