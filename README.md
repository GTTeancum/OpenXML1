...
Whole-block VU execution is now implemented and opt-in, with 54/54 focused tests passing and exact results over the 32 captured workloads, including short-cycle resumption. A complete runner-only non-IPO rebuild and link now produces a statically validated candidate under the unchanged 2 GiB memory limit while keeping runtime/native-kernel optimization enabled. The candidate has not been run yet, so live behavior and frame rate remain unverified. Offline improvements and build success are not a new gameplay FPS result. See [the current resume point](xmen-legends/TODO.md) and [validation notes](xmen-legends/VU-REPLAY.md).
