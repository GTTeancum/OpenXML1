...
This is not yet a playable-speed build: a gameplay sample still measured about 2.7 FPS. Broader timing identifies vector processing as the main remaining cost, so that is the next performance focus. Selected-source builds now rearchive their static library before relinking; previous performance candidates built without that step are not reliable evidence.
