...
This is not yet a playable-speed build: the latest ordinary build measured about 2.75 FPS over 128 gameplay frames, without performance profilers enabled. Broader timing identifies vector processing as the main remaining cost. A small operand-preparation optimization is submitted as [PS2Recomp PR #247](https://github.com/ran-j/PS2Recomp/pull/247), with 427/427 upstream-based tests and 93/93 local vector tests passing. Its synthetic arithmetic workload is about 5% faster with identical results, but a meaningful overall gameplay improvement is not established. Detailed per-instruction profiling is compiled out of ordinary builds. Selected-source builds now rearchive their static library before relinking; previous performance candidates built without that step are not reliable evidence.
