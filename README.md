...
The September 5 candidate integrates verified work through PS2Recomp `053b1bc`, including overlapping-register-write correctness, flag packing, native normalized-operand reuse, and removal of redundant debug-buffer initialization. It reaches textured New York and Wolverine through the real New Game handler. The latest unchanged-build rerun measured **2.51 FPS** over 128 frames, after earlier 3.98 and 4.14 FPS samples. This is still not practical playability, and these shared-host runs do not establish a sustained improvement. The candidate remains opt-in and the saved primary/staged builds are unchanged. Black props, missing foliage, and HUD/effect defects remain.

The latest gameplay profile attributes about **64.0%** of measured wall time to VU processing and **24.3%** to graphics submission/drawing; waiting is negligible. Gameplay-only coverage shows **33.8% of vector instruction pairs still use the interpreter**, while 45.3% run in compiled blocks and 20.9% in smaller native kernels. These coverage counts are not time shares. Practical frame rate remains the main unfinished requirement. The latest normal build passes **136/136 VU-related tests** and both private captures at normal and 1/8/16/64-cycle slicing. This is not a full-suite pass or proof of gameplay fidelity.

Both doubling the smaller native-kernel selection and reranking its existing 64 slots made recorded workloads slower, so neither selection was integrated into the game build. Removing redundant clearing of a 2.4 KiB diagnostic buffer passed exact trace/replay checks but did not establish a gameplay-speed gain. Subsequent operand-reuse and deferred-writeback experiments were removed after unreliable or mixed timing results; their regression coverage is retained. Fresh profiles are guiding the next execution changes. Detailed results are in [VU-REPLAY.md](xmen-legends/VU-REPLAY.md).
