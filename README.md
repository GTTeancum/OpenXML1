...
This is not yet a playable-speed build. A controlled same-executable comparison over 128 gameplay frames measured **3.80 FPS with native VU blocks versus 3.39 FPS interpreted**, about a 12.1% improvement. These are sequential shared-machine samples rather than a sustained-speed guarantee, and existing black-prop, missing-foliage, HUD, and effect defects remain. Broader timing identifies vector processing as the main remaining cost. A small operand-preparation optimization is submitted as [PS2Recomp PR #247](https://github.com/ran-j/PS2Recomp/pull/247), with 427/427 upstream-based tests. Removing duplicate cycle work, improving packet copying, and replacing full integer-register scans with existing masks is submitted as [PS2Recomp PR #248](https://github.com/ran-j/PS2Recomp/pull/248), with 427/427 upstream-based tests. The current native-block bring-up passes all 122 VU-related tests; this is not a full-suite pass.
...
Whole-block VU execution is now implemented and opt-in, with exact results over the 32 captured workloads, including short-cycle resumption. The active block set retires 58,870 of 80,194 replayed instruction pairs natively. It can continue through bounded PATH1 transfers and pending delayed integer-register writes while preserving their original cycle visibility. A complete runner-only non-IPO rebuild and link produces the validated candidate under the unchanged 2 GiB memory limit while keeping runtime/native-kernel optimization enabled. The candidate reached textured first-level gameplay and exited normally in both measured modes. See [the current resume point](xmen-legends/TODO.md) and [validation notes](xmen-legends/VU-REPLAY.md).
