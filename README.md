...
The latest gameplay profile attributes about **63.0%** of measured wall time to VU processing and **25.4%** to graphics submission/drawing; waiting is negligible. Gameplay-only coverage shows **33.8% of vector instruction pairs still use the interpreter**, while 45.3% run in compiled blocks and 20.9% in smaller native kernels. These coverage counts are not time shares. Practical frame rate remains the main unfinished requirement. The latest source passes **131/131 VU-related tests**; the arithmetic changes passed both private captures at normal and 1/8/16/64-cycle slicing, and the subsequent reporting change preserves both captures with reporting enabled. This is not a full-suite pass or proof of gameplay fidelity.
