...
The September 5 candidate integrates verified work through PS2Recomp `304e2a1`, including overlapping-register-write correctness, flag packing, native normalized-operand reuse, and removal of redundant debug-buffer and graphics-packet initialization. It reaches textured New York and Wolverine through the real New Game handler. The latest candidate measured **4.06 FPS** over 128 frames and exited normally; the preceding unchanged older-candidate run measured 3.71 FPS. This is still not practical playability, and these shared-host runs do not establish a sustained improvement. The candidate remains opt-in and the saved primary/staged builds are unchanged. Black props, missing foliage, and HUD/effect defects remain.
