...
The September 5 candidate integrates the verified native-block work through PS2Recomp `119706f`, including overlapping-register-write correctness, flag packing, and normalized operand reuse. The latest completed test reaches textured New York and Wolverine through the real New Game handler and measures **4.14 FPS** over 128 frames. This is still not practical playability and is not a controlled improvement over earlier 3.11 or 3.97 FPS observations. The candidate remains opt-in and the saved primary/staged builds are unchanged. Black props, missing foliage, and HUD/effect defects remain.

The latest gameplay profile attributes about **64.0%** of measured wall time to VU processing and **24.3%** to graphics submission/drawing; waiting is negligible. Gameplay-only coverage shows **33.8% of vector instruction pairs still use the interpreter**, while 45.3% run in compiled blocks and 20.9% in smaller native kernels. These coverage counts are not time shares. Practical frame rate remains the main unfinished requirement. The latest diagnostic build passes **133/133 VU-related tests** and both private captures at normal and 1/8/16/64-cycle slicing. This is not a full-suite pass or proof of gameplay fidelity.

Doubling the smaller native-kernel selection made both recorded workloads slower, so that experiment was rejected without changing the game build. A new opt-in replay diagnostic measures instructions still executed outside compiled blocks. The next experiment will use those counts to choose better entries within the existing 64-kernel limit, rather than increasing the build size again. No new gameplay-speed gain is claimed.
