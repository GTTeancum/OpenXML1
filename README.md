...
The September 5 candidate integrates verified work through PS2Recomp `053b1bc`, including overlapping-register-write correctness, flag packing, normalized operand reuse, and removal of redundant debug-buffer initialization. The latest completed test reaches textured New York and Wolverine through the real New Game handler and measures **3.98 FPS** over 128 frames. This is still not practical playability; the earlier 4.14 FPS sample and this shared-host run do not establish a sustained improvement. The candidate remains opt-in and the saved primary/staged builds are unchanged. Black props, missing foliage, and HUD/effect defects remain.

The latest gameplay profile attributes about **64.0%** of measured wall time to VU processing and **24.3%** to graphics submission/drawing; waiting is negligible. Gameplay-only coverage shows **33.8% of vector instruction pairs still use the interpreter**, while 45.3% run in compiled blocks and 20.9% in smaller native kernels. These coverage counts are not time shares. Practical frame rate remains the main unfinished requirement. The latest normal build passes **134/134 VU-related tests** and both private captures at normal and 1/8/16/64-cycle slicing. This is not a full-suite pass or proof of gameplay fidelity.

Both doubling the smaller native-kernel selection and reranking its existing 64 slots made recorded workloads slower, so neither selection was integrated into the game build. A follow-up removed redundant clearing of a 2.4 KiB diagnostic buffer on every VU run. Exact trace and replay checks pass; its normal-budget replay gains are small, while 8-cycle slices improve about 5-7%. The completed game check still runs at roughly four FPS, so this is not yet a meaningful gameplay-speed gain.
